...
/**
 * RecentProjectsCard Component
 *
 * Dashboard widget showing recent projects.
 * Gracefully handles the Projects API not being ready yet.
 *
...
export default function RecentProjectsCard() {
  // Projects API is a stub (Member 3's module).
  // When ready, this will fetch from projectService.getAllProjects() and display a list.

  return (
    <div className="card border-[#2d333b]">
      <div className="flex items-center justify-between mb-4">
        <h2 className="font-sans font-bold text-[#eaedf0] text-sm">💼 Recent Projects</h2>
        <Link
          href="/projects"
      ...
      <div className="flex flex-col items-center justify-center text-center py-8 px-4 rounded-sm border border-dashed border-[#2d333b] bg-gradient-to-b from-[#15181d] to-[#1a1e24] transition-colors hover:border-[#475363]">
        ...
        <p className="font-sans font-semibold text-sm text-[#eaedf0] mb-1">Projects Module Pending</p>
        <p className="font-mono text-[10px] text-[#7a889b] mb-4 max-w-[220px]">
          Your recent projects will appear here once the module is active.
        </p>
        <Link href="/projects" className="btn-secondary text-[10px] px-4 py-1.5 opacity-80 hover:opacity-100 transition-opacity">
          Browse Projects
        </Link>
      </div>
    </div>
